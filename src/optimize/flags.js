...
const DRAWING_COMMANDS = HORIZ_LINE_TO | VERT_LINE_TO | LINE_TO | CURVE_TO | SMOOTH_CURVE_TO | QUAD_TO | SMOOTH_QUAD_TO | ARC;
        ...
        if (currentNumber !== '' && currentCommandType) {
            ...
            if (currentCommandType === ARC) {
                if (currentArgs.length < 2 && value <= 0) {
                    if (debug) {
                        console.log('Command: "' + currentCommand + '", args: ', currentArgs);
                    }
