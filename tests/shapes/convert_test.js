...
(() => {
	...
	function Compare(data) {
		return new Promise((fulfill, reject) => {
			...
			// Execute
			let params = [
				__dirname + '/compare_svg_script.js',
				sourceFile,
				targetFile,
			];
			...
			exec(
				'phantomjs ' + params.map(param => '"' + param + '"').join(' '),
				{
					cwd: __dirname,
				},
				(error, stdout, stderr) => {
	...
	describe('Testing converting SVG shapes to path', () => {
				...
				let test = tests[key],
					body = [
						'<' +
							test.shape +
							' ' +
							mergeAttributes(test.attributes) +
							mergeAttributes(extraAttributes) +
							'/>',
						'<path d="' +
							Convert[test.shape](test.attributes) +
							'" ' +
							mergeAttributes(extraAttributes) +
							'/>',
					];
		...
		it('rectangle', () => {
			...
			expect(results.match).to.be.equal(
				true,
				'Rectangle and path are not the same. See _' + key + '.html'
			);
		});
		...
		it('circle', () => {
			...
			expect(results.match).to.be.equal(
				true,
				'Circle and path are not the same. See _' + key + '.html'
			);
		});
		...
		it('ellipse', () => {
			...
			expect(results.match).to.be.equal(
				true,
				'Ellipse and path are not the same. See _' + key + '.html'
			);
		});
		...
		it('polygon', () => {
			...
			expect(results.match).to.be.equal(
				true,
				'Polygon and path are not the same. See _' + key + '.html'
			);
		});
		...
		it('polyline', () => {
			...
			expect(results.match).to.be.equal(
				true,
				'Polyline and path are not the same. See _' + key + '.html'
			);
		});
	});
})();
